...
| ORM | Drizzle | 見 [decisions_v2.md](decisions_v2.md) ADR-001 |
| 認證 | NextAuth.js (Auth.js) | OAuth 2.0 + 2FA (TOTP) |
| 部署 | Docker Compose | Self-hosted |

> **P0 不含 Notion**。跳過 Notion MVP，直接在目標技術棧上構建最小 Web App。Notion 相關設計保留於 [desktop_research_v2.md](desktop_research_v2.md) §10 作為歷史參考。
...
| 狀態管理 | **TanStack Query** (服務器狀態) + **Zustand** (客戶端 UI 狀態) | TanStack Query 負責 API 數據快取/同步，Zustand 負責主題/幣別選擇等純 UI 狀態（見 [decisions_v2.md](decisions_v2.md) ADR-002） |
...
| 組件 | 技術 | 理由 |
|------|------|------|
| API | **Next.js API Routes** (P0) → **tRPC** (P1+) | P0 直接用 API Routes，穩定後遷移 tRPC 獲得端到端型別安全（見 [decisions_v2.md](decisions_v2.md) ADR-004） |
| ORM | **Drizzle** (推薦) | 金融場景需要 Decimal 精度，Drizzle 可以直接映射 PostgreSQL `numeric` 類型，避免浮點數誤差（見 [decisions_v2.md](decisions_v2.md) ADR-001） |
...
CREATE INDEX idx_exchange_rates_lookup ON exchange_rates(base_currency, quote_currency, snapshot_date);
```
...
- **數據模型**：採用 **Holding-centric** 模型（見 [decisions_v2.md](decisions_v2.md) ADR-006）。Holdings 為一等公民，Transactions 為附屬記錄。
- **索引**：為常用查詢路徑定義索引，保障 Dashboard 載入 < 3 秒的性能指標。
...
│   └── mcp-server/       # Python MCP Server (P3+)
├── packages/
│   └── shared/           # 共享型別、常數、工具函數
├── docker-compose.yml
├── Caddyfile
...
> **語言決策**：採用 **Python SDK** 開發 MCP Server，利用 Python 豐富的金融數據處理生態（pandas, numpy），與主流金融 API SDK 相容性佳（見 [decisions_v2.md](decisions_v2.md) ADR-005）。此為 P3+ 功能。
